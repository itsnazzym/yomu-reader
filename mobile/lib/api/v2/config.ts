/**
 * nhentai API v2 — Configuration / Proof of Work / Captcha / CDN
 *
 * GET /api/v2/pow      Proof-of-Work challenge
 * GET /api/v2/captcha  Captcha provider info
 * GET /api/v2/config   CDN hostnames
 */
...
/** Get a Proof-of-Work challenge. Action: "login" | "comment" | "register" | "reset" */
...
const FALLBACK_TURNSTILE_KEY = "0x4AAAAAABmfxjJLaRcDE3KW";
...
export async function getCaptchaInfo(): Promise<CaptchaInfo> {
  try {
    ...
    return {
      ...
      site_key: site_key || FALLBACK_TURNSTILE_KEY,
    };
  } catch {
    return {
      provider: "turnstile",
      site_key: FALLBACK_TURNSTILE_KEY,
    };
  }
}
